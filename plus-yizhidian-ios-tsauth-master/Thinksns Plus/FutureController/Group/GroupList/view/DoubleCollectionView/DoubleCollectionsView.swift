...
//    let topCollcetion = UICollectionView(frame: CGRect(origin: .zero, size: CGSize(width: UIScreen.main.bounds.width - 40, height: 44)), collectionViewLayout: TopTagCollectionLayout())
    // 下方集合视图
    var bottomCollcetion = UICollectionView(frame: CGRect(x: 0, y: 0, width: UIScreen.main.bounds.width, height: UIScreen.main.bounds.height), collectionViewLayout: BottomCollectionLayout())
    // 分割线
...
//        topCollcetion.register(TagCollectionCell.self, forCellWithReuseIdentifier: TagCollectionCell.identifier)
        ...
        bottomCollcetion.backgroundColor = UIColor.clear
...
class BottomCollectionLayout: UICollectionViewFlowLayout {
    ...
    override func prepare() {
        super.prepare()
        // 1.设置 item 大小
        itemSize = CGSize(width: UIScreen.main.bounds.width, height: UIScreen.main.bounds.height - 64)
        // 2.设置间距
        minimumLineSpacing = 0
        minimumInteritemSpacing = 0
        // 3.设置滚动方向
